...
class StanceModelT5Gen(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output = {"__key__": batch["__key__"],
                  ...
                  "task_losses": {"Stance": outputs.loss.detach().cpu().item()},
    ...
    def predict_step(self, batch, batch_idx):
        ...
        input_ids = batch["json"]["encoded"]["input_ids"]
        #preds = self.model.predict_from_input_ids(input_ids)
        outputs = self.get_model_outputs(batch)
...
class T5ForSequenceClassification(nn.Module):
    ...
    def __init__(self, pretrained_model_name_or_path, label_spec,
        ...
        if len(label_spec.keys()) > 1:
            raise ValueError("T5 model can only handle single-task classification.")
        ...
        # TODO: why does the model perform worse if we use "reject" instead of "deny"?
    ...
    def predict_from_input_ids(self, input_ids):
        # TODO: For some reason using generate to predict performs much worse than 
        # predicting directly from logits as below.
