...
from .attention import SparsegenLin, Fusedmax
...
class MyMT5Attention(nn.Module):
    def __init__(self, config: MT5Config, has_relative_attention_bias=False,
                 projection_fn="softmax", projection_fn_kwargs=None):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.has_relative_attention_bias = has_relative_attention_bias
        self.projection_fn_kwargs = projection_fn_kwargs
        if self.projection_fn_kwargs is None:
            self.projection_fn_kwargs = {}
        self.projection_fn = self._get_projection_fn(projection_fn)
    ...
    def _get_projection_fn(self, projection_fn_str):
        if projection_fn_str == "softmax":
            return nn.Softmax(dim=-1)
        elif projection_fn_str == "fusedmax":
            return Fusedmax(**self.projection_fn_kwargs)
        elif projection_fn_str == "sparsegen-lin":
            return SparsegenLin(dim=-1, **self.projection_fn_kwargs)
        else:
            raise KeyError(f"Unsupported projection_fn '{projection_fn_str}'")
    ...
    def forward(
        ...
        attn_weights = self.projection_fn(scores.float()).type_as(scores)
...
class MT5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = MyMT5Attention(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = MT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class MyMT5LayerCrossAttention(nn.Module):
    def __init__(self, config, projection_fn="softmax", projection_fn_kwargs=None):
        super().__init__()
        self.EncDecAttention = MyMT5Attention(
            config, has_relative_attention_bias=False,
            projection_fn=projection_fn,
            projection_fn_kwargs=projection_fn_kwargs)
        self.layer_norm = MT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class MT5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        if self.is_decoder:
            self.layer.append(MyMT5LayerCrossAttention(config))
...
class MT5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, MT5LayerNorm):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, MyMT5Attention):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (MyMT5Attention, MT5Stack)):
...
class MT5ForExplainableSequenceClassification(MT5ForConditionalGeneration):
    ...
    def __init__(self, config: MT5Config):
        ...
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = MT5Stack(decoder_config, self.shared)
        # Replace the final cross attention layer with a single-headed sparse version.
        cross_attn_config = copy.deepcopy(decoder_config)
    ...
    def forward(self, *args, **kwargs):
        kwargs["cross_attn_head_mask"] = self.cross_attn_head_mask
        return MT5ForConditionalGeneration.forward(self, *args, **kwargs)
